...
// Creates a new fork of app process with a given config
// Useful for tests that want to verify certain config options
function forkApp(newConfig, envName) {
    ...
    .then(function (port) {
      ...
      return new Promise(function (resolve, reject) {
        fs.writeFile(newConfigFile, 'module.exports = {' + envName + ': ' + JSON.stringify(newConfig) + '}', function (err) {
          ...
          child.port = port;
          // periodic check until forked App is running and is listening on the port
