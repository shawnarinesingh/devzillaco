...
sendInviteEmail = function sendInviteEmail(user) {
  ...
  return Promise.join(
    ...
    emailData.resetLink = baseUrl.replace(/\/$/, '') + '/signup/' + globalUtils.encodeBase64URLsafe(resetToken) + '/';
  ...
  browse: function browse(options) {
    ...
    function handlePermissions(options) {
      return canThis(options.context).browse.user().then(function permissionGranted() {
        ...
        return errors.handleAPIError(error, 'You do not have permissions to browse users.');
      });
    }
    
    /**
     * ### Model Query
     * Make a call to the Model layer
     * @param {Object} options
     * @returns {Object} options
     */
  ...
  edit: function edit(object, options) {
    ...
    function handlePermissions(options) {
      ...
      return canThis(options.context).edit.user(options.id).then(function () {
        // if roles aren't in the payload, process with the edit
