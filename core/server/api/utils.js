...
utils = {
  ...
  // data DefaultOptions - valid for all endpoints which take object as well as options
  ...
  validate: function validate(docName, extras) {
    ...
    return function doValidate() {
       ...
       * - incorrect options keys are sanitized
    ...
    _.each(options, function (value, key) {
      ...
      if (noValidation.indexOf(key) === -1) {
        if (globalValidations[key]) {
          ...
          // all other keys should be an alphanumeric string + -, like slug, status, etc.
