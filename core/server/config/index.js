...
ConfigManager.prototype.init = function (rawConfig) {
  ...
  return Promise.all([requireTree(self._config.paths.templatesPath), requireTree(self._config.paths.appPath)]).then(function (paths) {
    self._config.paths.availableTemplates = paths[0];
...
ConfigManager.prototype.set = function (config) {
  ...
  // Special case for the template.navigation JSON object, which should be overridden not merged
  if (config && config.templates && config.templates.navigation) {
    this._config.templates.navigation = config.templates.navigation;
  }
  ...
  // @todo: perhaps put access of config object behind a function?
