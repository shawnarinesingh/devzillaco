...
checks = {
  ...
  nodeVersion: function checkNodeVersion() {
    ...
    try {
      ...
      if (!semver.satisfies(process.versions.node, packages.engines.node) &&
        ...
        console.error('\x1B[31mNeed the Node version ' + packages.engines.node +
  ...
  packages: function checkPackages() {
    ...
    console.error('\x1B[32mHelp and documentation can be found at http://support.devzilla.co.\033[0m\n');
  ...
  contentPath: function checkContentPaths() {
    ...
    var configFile,
        config,
        contentPath,
        contentSubPaths = ['apps', 'data', 'images', 'templates'],
        fd,
        errorHeader = '\x1B[31mERROR: Unable to access content path:\033[0m',
        errorHelp = '\x1B[32mCheck that the content path exists and file system permissions are correct.' +
                    '\nHelp and documentation can be found at http://support.devzilla.co.\033[0m';
  ...
  sqlite: function checkSqlite() {
    ...
    try {
      ...
      dbPath = path.join(appRoot, 'content', 'data', mode === 'production' ? 'data.db' : 'data-dev.db');
    }
    ...
    try {
      ...
      // Database file not eixsting in not an error as sqlite will create it.
      if (e.code === 'ENOENT') {
        return;
      }
      ...
      console.error('Help and documentation can be found at http://support.devzilla.co.\033[0m');
