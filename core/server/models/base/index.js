...
// accesses the models directly. All other parts, including the blog frontend, admin UI, and apps are only
...
appBookshelf = bookshelf(config.database.knex);

// Load the Bookshelf registry plugin, which help us avoid circular dependencies
appBookshelf.plugin('registry');

// Load the pagination plugin, which gives us the `fetchPage` method on Models
appBookshelf.plugin(pagination);

// ## appBookshelf.Model
// The Base Model which other objects will inherit from,
...
appBookshelf.Model = appBookshelf.Model.extend({
  ...
  /**
   * Filter potentially unsafe `options` in a model method's arguments, so you can pass them to Bookshelf / Knex.
  ...
  */
  generateSlug: function generateSlug(Model, base, options) {
    ...
    if (baseName === 'user' && options && options.shortSlug && slugTryCount === 1 && slug !== 'owner') {
      longSlug = slug;
